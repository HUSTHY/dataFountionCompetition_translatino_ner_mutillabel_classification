...
from utils.adversarial_model import FGM, PGD, FreeLB
from collections import defaultdict
from torch.utils.tensorboard import SummaryWriter
from model.ema import EMA

log_file = 'log_files/'+'v22_k_5fold_MLCE_L512_bert_rdrop_0_FGM_ema_semisupervised_1.log'
...
def parse_args():
    ...
    parser.add_argument("--version", type=str, default='v22', help="epochs")
...
def train(args):
    ...
    for fold, (train_index, dev_index) in enumerate(skf.split(df)):
        ...
        if args.adversarial:
            if args.adversarial_type == 'FGM':
                ...
                freelb = FreeLB(multilabel_crossentropy, adv_K=3, adv_lr=1e-2, adv_init_mag=2e-2)
        ...
        for epoch in range(args.epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.adversarial and args.adversarial_type != "FreeLB":
                    loss.backward()
                else:
                    loss.backward()
                ...
                if args.adversarial:
                    if args.adversarial_type == "FGM":
                        ...
                        loss = freelb.attack(model,inputs,labels)
        ...
        ids = test_df['id']
        texts = test_df['text']
        submit_dir = 'submit/5_fold_20211203/'
        if not os.path.exists(submit_dir):
            os.makedirs(submit_dir)
        submit_path = submit_dir + 'submition_' + args.loss_function_type + '_L' + str(args.max_len) + '_bert_FGM_' + str(fold) + '.json'
        print(save_path)
    ...
    file_name = 'submit/' + args.version + '_' + args.model_type + '_submition_'+ args.loss_function_type + '_L' + str(args.max_len) + '_rdrop_' + str(
        args.alpha) + '_RRL_scheduler_semi_supervised_1_' + args.adversarial_type + '_vote_integrate_vote_alpha_' + str(vote_alpha) + '_max_' + str(count) + '_doublecopy_20211203_seed5.json'
