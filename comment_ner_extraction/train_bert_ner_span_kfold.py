...
MODEL_CLASSES = {
    ...
    'roberta_large':(BertConfig, BertSpanForNer, CNerTokenizer),
    'ernie': (BertConfig, BertSpanForNer, CNerTokenizer),
    'albert': (AlbertConfig, AlbertSpanForNer, CNerTokenizer)
}


def evaluate(args, model, dev_dataloader,dev_feature, prefix=""):
    metric = SpanEntityScore(args.id2label)
    # Eval!
    logger.info("***** Running evaluation %s *****", prefix)
    logger.info("  Num examples = %d", len(dev_feature))
    ...
    for step, batch in enumerate(dev_dataloader):
        ...
        inputs = {"input_ids": batch[0], "attention_mask": batch[1], "start_positions": batch[3],
        ...
        # 修改成batch处理
        Rs = bert_extract_item_batch(start_logits, end_logits, input_lens)

        interval = batch[0].shape[0]
        Ts = []
        for f in dev_feature[count:(count+interval)]:
            subjects = f.subjects
            Ts.append(subjects)
        count += interval
...
def main():
    ...
    logger = init_logger(log_file='log/' + f'v20211119_{args.model_type}_span_5kfold_lsr_FGM_roberta_{time_}.log')
    args.logger = logger
    ...
    for fold, (train_index, dev_index) in enumerate(skf.split(df,labels)):
        ...
        args.model_save = os.path.join(args.output_dir, 'v0_' + args.run_time, '5fold_span_lsr_FGM_roberta_supervised_clean', str(fold))
        ...
        dev_dataset,dev_feature = load_and_cache_examples(args, dev_save_path, args.task_name, tokenizer, data_type='dev')
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                scheduler.step()
            _, avg_f1 = evaluate(args,model, dev_dataloader,dev_feature)
    ...
    path = 'result/v20211119_v0_Submission_rdrop_4.0_bert_cls_dropout_classficaition_' + args.model_type + '_' + args.integrate_type +'_5_span_ner_lsr_FGM_clean.csv'
        ...
        for save_path in model_save_paths:
            ...
            with torch.no_grad():
                ...
                for step, batch in enumerate(test_dataloader):
                    ...
                    # 修改成batch处理
                    Rs = bert_extract_item_batch(start_logits, end_logits,input_lens)
                    for R,length in zip(Rs,input_lens):
                        if R:
                            label_entities = [[args.id2label[x[0]], x[1], x[2]] for x in R]
                        else:
                            label_entities = []
                        each_model_predictlabels.append((label_entities,length-2))
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(test_dataloader):
                ...
                for i,model in enumerate(models):
                    ...
                    #起始位置概率融合
                    if i == 0:
                        start_logits_all = start_logits * weight
                        end_logits_all = end_logits * weight
                    else:
                        start_logits_all += start_logits * weight
                        end_logits_all += end_logits * weight
                    weight_sum += weight

                start_logits_all = start_logits_all/weight_sum  # 注意要平均
                end_logits_all = end_logits_all / weight_sum  # 注意要平均
