            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1:
                        ...
                        _,avg_f1,_,_=evaluate(args, model, tokenizer)
                        ...
                        if train_max_f1 < avg_f1:
                            ...
                            if args.local_rank in [-1,0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                                ...
                                output_dir = os.path.join(args.output_dir, args.run_time+"_best_model_checkpoint")
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        for i, label in enumerate(out_label_ids):
            ...
            if len(temp_1)>0 and temp_1 != temp_2:
                index = args.eval_batch_size*step+i
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        batch = tuple(t.to(args.device) for t in batch)
        with torch.no_grad():
            # print("input_ids ... batch_0",batch[0])
            # print("attention_mask ... batch_1",batch[1])
...
def main():
    ...
    # args.run_time = datetime.now().strftime('%Y-%m-%d-%H_%M_%S')
    ...
    #         model.to(args.device)
    #         result,f1= evaluate(args, model, tokenizer, prefix=prefix)
    #         if global_step:
    ...
    #     model.to(args.device)
    #     result, f1,eval_dicts, error_indexs = evaluate(args, model, tokenizer, prefix=prefix)
    #     if global_step:
    #         result = {"{}_{}".format(global_step, k): v for k, v in result.items()}
    #     results.update(result)
    ...
    #             writer.write("{} = {}\n".format(key, str(results[key])))
    #         writer.write('\n')
    #         for key,val in eval_dicts.items():
    #             writer.write("{}:{}\n".format(key, str(val)))
    #
    #     with open('add_three_level_data/old_devset.txt','r',encoding='utf-8') as f:
    #         lines = f.readlines()
    #
    #     output_eval_error_file = os.path.join(output_dir, "eval_error.txt")
    #     with open(output_eval_error_file, "w") as writer:
    #         for index,line in enumerate(lines):
    #             if index in error_indexs:
    #                 writer.write(line)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = model_class.from_pretrained(output_dir, config=config)
        model.to(args.device)
        results = predict(args, model, tokenizer, prefix=prefix)

        import pandas as pd
        df = pd.read_csv('result/v0_Submission_rdrop_4.0_bert_classficaition.csv')
        predicts_entities = []

        assert len(df) == len(results)
        for result in results:
            pre_label = result['tag_seq']
            predicts_entities.append(pre_label)

        df['BIO_anno'] = predicts_entities

        df.to_csv('result/v0_Submission_rdrop_4.0_bert_classficaition_crf_ner.csv',index=False)
